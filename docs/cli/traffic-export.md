...
**Note**: When `--output-file` is not specified, a timestamped filename is automatically generated in the format: `traffic-export_YYYYMMDD-HHMMSS.<format>`. Use `--output-file-timestamp` to add timestamps when providing a custom filename.
...
pylo traffic-export \
  --timeframe-hours 24 \
  --format csv \
  --output-file ./reports/traffic.csv
```
...
pylo traffic-export \
  ...
  --format excel \
  --output-file ./exports/traffic.csv \
